...
def output_average_dist_by_method(data, num, testcase, method):
    ...
    """
    if method not in ['Dtree', 'nDtree', 'HK', 'ET', 'opt','HDT']:
...
def update_res_query_Sd(testcase, result_type, data, method):
    ...
    if not Path(filename).exists():
        writer = open(filename, 'w')
        if is_small_graph:
            writer.write('ts,Dtree,nDtree,HK,HDT,ET,opt\n')
        else:
            writer.write('ts,Dtree,nDtree,HK,HDT\n')
        writer.flush()
        writer.close()
...
def update_average_runtime(testcase, type, data, method):
    ...
    with open(filename, 'r', newline = '') as csvFile, tempfile:
        ...
        if latest_time < current_time:
            if is_small_graph:
                items = [current_time, 0, 0, 0, 0, 0, 0]
            else:
                items = [current_time, 0, 0, 0, 0]
            items[index] = res
            writer.writerow(items)
