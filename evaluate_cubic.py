...
if __name__ == '__main__':
    ...
    while current_time <= test_points[-1]:
        ...
        while idx < len(records) and records[idx][2] == current_time:
            ...
            if records[idx][3] == '+':
                ...
                if root_a != root_b:
                    ...
                    start = timer()
                    dg.addTreeEdge(HDT_graph, a, b)
                    HDT_res.in_te_time += (timer() - start + go_to_root_HDT)
                else:  # a and b are connected
                    if not dg.isTreeEdge(HDT_graph, a, b):
                        # (a, b) is a new  non tree edge
                        # inserting a non tree edge
                        HDT_res.in_nte_count += 1
                        # count running time for inserting a non tree edge in DT
                        start = timer()
                        dg.addNonTreeEdge(HDT_graph, a, b)
                        HDT_res.in_nte_time += (timer() - start + go_to_root_HDT)
                ...
                if (a, b) not in inserted_edge:
                    print("ERROR: removal of non-edge")
                    exit -1
                ...
                # HDT
                if dg.isEdge(HDT_graph, a, b):
                   if dg.isTreeEdge(HDT_graph, a, b):
                       start = timer()
                       dg.remTreeEdge(HDT_graph, a, b)
                       HDT_res.de_te_time += (timer() - start)
                       HDT_res.de_te_count += 1
                   else: 
                       start = timer()
                       dg.remNonTreeEdge(HDT_graph, a, b)
                       HDT_res.de_nte_time += (timer() - start)
                       HDT_res.de_nte_count += 1
        ...
        if current_time in test_points:
            ...
            #update_maintanence(testcase, HDT_res, 'HDT')
